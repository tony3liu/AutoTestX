...
- [x] **步骤 1：定义 TypeScript 类型 (`src/types/test.ts`)**
定义 `TestCase`、`Assertion`、`TestResult` 和 `TestSuite`。

- [x] **步骤 2：在主进程实现数据库的 IPC 处理程序**
暴露 `ipcMain.handle` 端点，例如 `test:createCase`，`test:listCases`。

- [x] **步骤 3：实现前端状态管理 Store (`src/stores/test-store.ts`)**
