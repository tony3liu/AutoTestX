...
2. **启动开发环境**：
   运行以下命令即可同时启动 AutoTestX 桌面客户端和附带的本地网关服务：
   ```bash
   pnpm dev
   ```

3. **配置 API 密钥**：
   AutoTestX 需要大语言模型 (LLM) 的支持。你可以通过以下两种方式进行配置：
   
   - **方式 A (推荐)：** 启动应用后，在侧边栏进入 **“模型设置”** (Model Settings)，在界面上直接填入你的提供商 (如 OpenAI, Claude) 的 API Key 和模型配置。
   - **方式 B (环境变量)：** 启动前通过环境变量注入（适用于自动化部署或快速测试）：
     ```bash
