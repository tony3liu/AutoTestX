...
(function () {
  ...
  if (globalThis.__autotestxFetchPatched) return;
  globalThis.__autotestxFetchPatched = true;

  globalThis.fetch = function autotestxFetch(input, init) {
    var url =
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__autotestxPatched) {
        cp.__autotestxPatched = true;
